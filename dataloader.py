...
path = Path("/content/gdrive/MyDrive/MICCAI_BraTS2020_TrainingData")
...
parent_folders = (path).ls()

def get_parents(path:Path, shuffle:bool=True, pct=0.2):
  "List a certain percent of items under a specified parent directory randomly or not"
  from random import shuffle
  ps = [d[i] for r,d,_ in os.walk(path) for i in range(len(d))] 
...
val_list = get_parents(path,pct=0.5) #get_parents(path, 'LGG', pct=0.1)
